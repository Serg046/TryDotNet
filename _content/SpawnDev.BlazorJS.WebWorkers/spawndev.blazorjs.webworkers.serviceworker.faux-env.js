...
function createProxiedObject(obj) {
    var ret = new Proxy(obj, {
        get(target, key) {
            ...
            consoleLog('get', target, keyStr, typeofProp === 'object' && typeofProp.constructor ? typeofProp.constructor.name : typeofProp, typeofProp !== 'function' ? ret : '(){ ... }');
            if (propIsUndefined) {
        ...
        set(target, key, value) {
            consoleLog('set', key, value);
            target[key] = value;
            return true;
        },
        deleteProperty(target, key) {
            consoleLog('deleteProperty', target, key);
            if (!(key in target)) {
                return false;
            }
            delete target[key];
        ...
        ownKeys(target) {
            consoleLog('ownKeys', target);
            return Object.keys(target);
        },
        has(target, key) {
            consoleLog('has', target, key);
